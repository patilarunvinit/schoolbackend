from django.http import JsonResponse
from rest_framework.parsers import JSONParser
from django.views.decorators.csrf import csrf_exempt
from .serializers import sinfoserializer
from .models import sinfo



from django.contrib.auth import authenticate, login, logout
from rest_framework.authtoken.models import Token
from rest_framework.decorators import  permission_classes
from rest_framework.permissions import AllowAny
from rest_framework.status import (
    HTTP_400_BAD_REQUEST,
    HTTP_404_NOT_FOUND,
    HTTP_200_OK
)

# Create your views here.

@csrf_exempt
def stinfo(request):
    if request.method =='POST':
        data1 = JSONParser().parse(request)
        outdata = sinfoserializer(data=data1)
        print(outdata)
        if outdata.is_valid():
            outdata.save()
            return JsonResponse(outdata.data, safe=False)
        else:
            return JsonResponse(outdata.error, safe=False)

    elif request.method == "GET":
        sdata = sinfo.objects.all()
        s1data = sinfoserializer(sdata, many=True)
        return JsonResponse(s1data.data, safe=False)





@csrf_exempt
@permission_classes((AllowAny,))

def loginS(request):

   if request.method == "POST":
       data2 = JSONParser().parse(request)
       username = data2["username"]
       password = data2["password"]
       print(username,password)
       counter=sinfo.objects.filter(username=username)

       if username=="" or password =="":
           return JsonResponse({'error': 'Please provide both username and password'},
                           status=HTTP_400_BAD_REQUEST)
       user = authenticate(username=username, password=password)
       if not user:
           return JsonResponse({'error': 'Invalid Credentials'},
                           status=HTTP_404_NOT_FOUND)
       else:
           if counter:
               login(request, user)
               request.session['username'] = username
               request.session.save()
               token, _ = Token.objects.get_or_create(user=user)
               sdata = sinfo.objects.filter(username=username)
               s1data = sinfoserializer(sdata, many=True)
               return JsonResponse({'token': token.key, "massage": "logged in successfuly", "Sdata": s1data.data})

           else:
               return JsonResponse({'error': 'WRONG LOGIN'},
                                   status=HTTP_404_NOT_FOUND)



   elif request.method =='GET':

        data=sinfo.objects.all()
        output=sinfoserializer(data, many=True)
        return JsonResponse(output.data, safe=False)




@csrf_exempt
def slogout(request):
    if request.method =='GET':
        logout(request)
        try:
            del request.session['username']
        except KeyError:
            pass
        return JsonResponse({"massage": "logout successfuly"}, safe=False)




@csrf_exempt
def slogindataapi(request):
    if request.method =='GET':

        try:
            print("test1")
            username = request.session['username']
            print(username)
            data = sinfo.objects.filter(username="vinit")
            output = sinfoserializer(data, many=True)
            data[0]
            return JsonResponse(output.data, safe=False)

        except:

            return JsonResponse({"massage": "login 1st"}, safe=False)